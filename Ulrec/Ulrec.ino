...
int Seconds = 36;    // Amount of seconds you want to wait for. (Accounts for lag in time with run time of motor)
...
void loop() 
{
  ...
  for (angle = 0; angle < 180; angle += 1)   // Turns the Servo Motor to 180 degrees.
  {
    ...
    Serial.println("Begin Feeding Ulrec..."); // Prints that Charlie is being fed.
  ...
  for (angle = 180; angle >= 1; angle -= 1)    // Turns the Servo Motor back to 0 degrees. 
  {
    ...
    Serial.println("End Feeding Ulrec..."); // Prints Charlie has stopped being fed.
  ...
  {
    Serial.println("Yellow Light On"); // Prints that the Yellow light is on
    ...
    Serial.println("Stopping Timer");   // Prints that the timer is ending. 
...
void times()
{ 
  if (Seconds > 0)  // Checks if seconds is greater than 0.
  {
  ...
  {
    if (Minutes > 0)    // Checks if minutes is greater than 0.
    {
    ...
    {
      if (Hours > 0)  // Checks if hours is greater than 0
      {
        Seconds = 59;   // Sets seconds to 59. 
      ...
      {
        Seconds = 36;     // Resets seconds to 36. 
